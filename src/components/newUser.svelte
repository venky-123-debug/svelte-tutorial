<script>
  ...
  $: disableSubmitButton = !newUser.name || !newUser.mail
...
<div class="mt-4">
  ...
  <!-- {#if showModal} -->
  <NewUserModal on:close={() => (showModal = false)} on:submit={handleForm} show={showModal}>
    ...
    <button disabled={disableSubmitButton} type="submit" id="submit-button" slot="right-button" class="rounded bg-blue-800 px-2 py-1 text-white ">Create</button>
  </NewUserModal>
  <!-- {/if} -->
</div>
